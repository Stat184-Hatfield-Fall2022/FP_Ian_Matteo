...
```{r wrangling}
FinalTable <- FinalTable %>% mutate(`STANDING REACH` = parse_number (`STANDING REACH`))

FinalTable <- FinalTable %>% mutate(`HAND LENGTH (inches)` = parse_number(`HAND LENGTH (inches)`))

FinalTable <- FinalTable %>% mutate(`HAND WIDTH (inches)` = parse_number(`HAND WIDTH (inches)`))

FinalTable <- FinalTable %>% mutate(`HEIGHT W/O SHOES` = parse_number(`HEIGHT W/O SHOES`))
...
wingspan <- FinalTable %>%
 ...
 ggplot() +
  aes(x = BPG, y = WINGSPAN, fill = POS) +
  geom_boxplot() +
  scale_fill_hue(direction = 1) +
  labs(
    title = "Wingspan VS Blocks per game (BPG)",
    fill = "Position"
  ) +
  theme_minimal() +
  scale_x_continuous(
...
# Standing reach
standingreach <- FinalTable %>%
  filter(POS %in% c("PG", "SG", "PF", "SF", "C")) %>%
  ggplot() +
  aes(x = BPG, y = `STANDING REACH`, fill = POS) +
  geom_boxplot() +
  scale_fill_hue(direction = 1) +
  labs(
    title = "Standing Reach VS Blocks per game (BPG)",
    fill = "Position"
  ) +
  theme_minimal() +
  scale_x_continuous(
...
height <- FinalTable %>%
  filter(POS %in% c("PG", "SG", "PF", "SF", "C")) %>%
  ggplot() +
  aes(x = BPG, y = `HEIGHT W/O SHOES`, fill = POS) +
  geom_boxplot() +
  scale_fill_hue(direction = 1) +
  labs(
    title = "Height VS Blocks per game (BPG)",
    fill = "Position"
  ) +
  theme_minimal() +
  scale_x_continuous(
...
R_height <- RookieTable %>%
  ...
  scale_fill_hue(direction = 1) +
  labs(
    title = "Rookie Height VS Blocks per game (BPG)",
    fill = "Position"
  ) +
  theme_minimal() +
  scale_x_continuous(
...
#rookie standing reach 
R_standingreach <- RookieTable %>%
  filter(POS %in% c("PG", "SG", "PF", "SF", "C")) %>%
  ggplot() +
  aes(x = BPG, y = `STANDING REACH`, fill = POS) +
  geom_boxplot() +
  scale_fill_hue(direction = 1) +
  labs(
    title = "Rookie Standing Reach VS Blocks per game (BPG)",
    fill = "Position"
  ) +
  theme_minimal() +
  scale_x_continuous(
...
ggarrange(R_standingreach, standingreach) + rremove("x.text")
```
